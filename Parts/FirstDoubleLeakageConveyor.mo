...
model FirstDoubleLeakageConveyor
  ...
  Physiolibrary.Types.RealIO.PopulationOutput deathUntested annotation (
      Placement(transformation(extent={{44,2},{64,22}}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={90,10})));
  Physiolibrary.Types.RealIO.PopulationOutput deathTested annotation (Placement(
        transformation(extent={{26,-100},{46,-80}}),iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={90,-110})));
  ...
  Physiolibrary.Types.RealIO.PopulationChangeOutput untestedDeathRate
    annotation (Placement(transformation(
        extent={{-9,-9},{9,9}},
        rotation=0,
        origin={49,27}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={50,10})));
  Physiolibrary.Types.RealIO.PopulationChangeOutput testedDeathRate
    annotation (Placement(transformation(
        extent={{-9,-9},{9,9}},
        rotation=0,
        origin={73,-75}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={50,-110})));
equation
  ...
  connect(testedConveyor.deathPopulation, deathTested) annotation (Line(points={{18.54,
          -64.08},{18.54,-90},{36,-90}},        color={0,0,127}));
  connect(untestedConveyor.deathPopulation, deathUntested)
    annotation (Line(points={{17,35},{17,12},{54,12}}, color={0,0,127}));
  ...
  connect(untestedConveyor.testedFractionRate, testedFractionRate)
    annotation (Line(points={{-63,44},{-86,44}}, color={0,0,127}));
  connect(testedRateMeasure.port_b, testedConveyor.inflow) annotation (Line(
      points={{36.8,-2},{-78,-2},{-78,-41.04},{-58.98,-41.04}},
      color={0,127,127},
      thickness=1));
  connect(testedConveyor.deathRate, testedDeathRate) annotation (Line(
        points={{31.8,-64.08},{31.8,-75},{73,-75}}, color={0,0,127}));
  connect(untestedConveyor.deathRate, untestedDeathRate)
    annotation (Line(points={{30,35},{30,27},{49,27}}, color={0,0,127}));
...
end FirstDoubleLeakageConveyor;
